...
/*
 * 在old_value后插入新的键值对（键值对溢出的情况以及父子关系更新此处无需处理，后续可以在外部处理）
 * @return: size
 */
INDEX_TEMPLATE_ARGUMENTS
 ...
 *  eg: r1->[<invalid, p0>, <1, p1>, <2, p2>, <3, p3>, <4, p4>] ----MoveHalfTo--> r2[]
 ...
 * 建议：1.由于该结点属于内部结点，结点中元素迁移后需要对子元素的ParentPageId进行更新，即对子结点进行fetch，更新后再unpin
 * 2.更新size
 */
INDEX_TEMPLATE_ARGUMENTS
...
std::string B_PLUS_TREE_INTERNAL_PAGE_TYPE::ToString(bool verbose) const {
  ...
  while (entry < end) {
    ...
    os << std::dec << array_[entry].first.ToString();
    if (verbose) {
      os << "(" << array_[entry].second << ")";
    }
    ++entry;
  }
  return os.str();
