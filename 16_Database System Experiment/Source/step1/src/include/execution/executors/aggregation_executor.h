...
class SimpleAggregationHashTable {
 ...
 private:
	 //关系如下：
	 //AggregateValue表示一组记录进行各种聚合操作后产生的结果
	 //为什么存在hashmap：这里结果AggregateValue是以key值来进行索引的，即key值相同则会进入同一组
	 //key是如何产生的:一个tuple以keyOutShceam的形式留下分组所需的属性，输出为AggregateKey（例如group by sno中的sno）
	 //同时该tuple产生对应的value，用来计算聚合属性
