...
namespace bustub
{
  ...
  ValueType B_PLUS_TREE_INTERNAL_PAGE_TYPE::Lookup(const KeyType &key, const KeyComparator &comparator) const
  {
    ...
    {
      mid = (low + high) / 2;
      int flag = comparator(array_[mid].first, key);
      if (flag < 0)
        low = mid + 1;
      else if (flag > 0)
        high = mid - 1;
                                                  ...
                                                  BufferPoolManager *buffer_pool_manager)
  {
    ...
    recipient->CopyNFrom(array_ + left_size, GetSize() - left_size, buffer_pool_manager);
    SetSize(left_size);
  }
  ...
  void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyNFrom(MappingType *items, int size, BufferPoolManager *buffer_pool_manager)
  {
    ...
    for (int i = currentSize; i < currentSize + size; i++)
    {
      array_[i] = *items++;
                                                 ...
                                                 BufferPoolManager *buffer_pool_manager)
  {
    ...
    for (int i = 0; i < GetSize(); ++i)
    {
      recipient->array_[recipientSize + i].first = array_[i].first;
      recipient->array_[recipientSize + i].second = array_[i].second;
      auto childPage = buffer_pool_manager->FetchPage(array_[i].second);
      BPlusTreePage *child = reinterpret_cast<BPlusTreePage *>(childPage->GetData());
      child->SetParentPageId(recipientPageId);
      buffer_pool_manager->UnpinPage(array_[i].second, true);
    }
    recipient->SetSize(recipientSize + GetSize());
    SetSize(0);
  }
