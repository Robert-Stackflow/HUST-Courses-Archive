...
grant select,insert,update on gaussdb.client to client_manager;
-- (3) 授予client_manager对bank_card表拥有查询除银行卡余额外的select权限；
grant select(b_number,b_type,b_c_id) on gaussdb.bank_card to client_manager;
-- (4) 授预fund_manager对fund表的select,insert,update权限；
grant select,insert,update on gaussdb.fund to fund_manager;
