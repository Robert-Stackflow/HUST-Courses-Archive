...
void create_list_2(list **headp,int number);
void AddToTail(list ** pHead);
int main(void)
{
	list *head=NULL,*p,*before;
	FILE *in,*out; 
	int sum_number=0,number=0,flag=1,check_count=0,check_new,count=1;
	char check_num[20];
	while(flag!=5&&flag!=0)
{
	scanf("%d",&flag);
	...
	{
		p=head;
		in=fopen("d:\\stu.dat","w");
		while(p)
		{
			fprintf(in,"%s %s %d %d %d %d %d %.2f\n",p->num,p->name,p->score[0],p->score[1],p->score[2],p->score[3],p->sum,p->average);
			p=p->next;
		} 
	}
	if(flag==7)
	{
//		create_list_2(&head,sum_number);
		p=head;
	while(p)
		{
			p->sum=p->score[0]+p->score[1]+p->score[2]+p->score[3];
...
void create_list_2(list **headp,int number)
{
	...
	FILE *out=fopen("d:\\stu.dat","r");
	char num0[20],ch;
	char name0[10];
	float average;
	int score0[4],sum;//依次为English,math,physics,C成绩
	fscanf(out,"%s%s%d%d%d%d",num0,name0,&score0[0],&score0[1],&score0[2],&score0[3]);
	//将录入的信息存入临时数据域中 
